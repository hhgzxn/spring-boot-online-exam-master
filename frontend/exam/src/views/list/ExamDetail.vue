<template>
  <a-layout>
    <a-layout-header class="header" style="color: #fff">
      ...
      <span style="font-size:20px;margin-left: 0px;" v-if="examDetail.exam">
    ...
    <a-layout>
      <a-layout-sider id="scroller" width="190" :style="{background: '#444',overflow: 'auto', height: '100vh', position: 'fixed', left: 0 }">
        <a-menu
          mode="inline"
        ...
        >
          <a-sub-menu key="question_radio" >
            ...
            <a-menu-item v-for="(item, index) in examDetail.radioIds" :key="item" v-show="item!='-1'"  @click="getQuestionDetail(item)">
          ...
          <a-sub-menu key="question_check">
            ...
            <a-menu-item v-for="(item, index) in examDetail.checkIds" :key="item" @click="getQuestionDetail(item)">
              <a-icon type="eye" theme="twoTone" twoToneColor="#52c41a" v-show="item!='-1'"  v-if="answersMap.get(item)"/>
              题目{{ index + 1 }}
            </a-menu-item>
          </a-sub-menu>
        </a-menu>
      </a-layout-sider>
      <a-layout :style="{ marginLeft: '189px' }">
        <a-layout-content :style="{overflow: 'initial' }">
          <div :style="{ padding: '24px', background: '#fff'}" >
            ...
            <strong>{{ currentQuestion.type }} </strong> {{ (currentQuestion.name+"").substring(3) }}
            <br><br>
            ...
            <a-radio-group @change="onRadioChange" v-model="radioValue" v-if="currentQuestion.type === '单选题' || currentQuestion.type === '判断题'">
              <a-radio v-for="option in currentQuestion.options" :key="option.questionOptionId" :style="option.questionOptionContent.length>200 ? optionStyle2 :optionStyle" :value="option.questionOptionId">
                {{ option.questionOptionContent }}
              </a-radio>
              <a-button type="danger" ghost style="margin-right: 60px;" @click="submit(currentQuestion)">submit</a-button>
            </a-radio-group>

            <!-- 多选题 -->
            <a-checkbox-group @change="onCheckChange" v-model="checkValues" v-if="currentQuestion.type === '多选题'">
              <a-checkbox v-for="option in currentQuestion.options" :key="option.questionOptionId" :style="option.questionOptionContent.length>200 ? optionStyle2 :optionStyle" :value="option.questionOptionId">
                {{ option.questionOptionContent }}
              </a-checkbox>
              <a-button type="danger" ghost style="margin-right: 60px;" @click="submit(currentQuestion)">submit</a-button>
            </a-checkbox-group>
...
import { questionAdmin } from '../../core/icons'
...
export default {
  ...
  mounted () {
    ...
    document.onkeyup = function(e) {
      ...
      if (ev.keyCode === 16) {
        that.submit(that.currentQuestion)
      }
    }  
  },
  methods: {
    ...
    submit (currentQuestion) {
      if(currentQuestion.type === '单选题') {
        this.examDetail.radioIds.forEach((item, index) => {
          if(item === currentQuestion.id) {
            if(index+1 == this.examDetail.radioIds.length) {
              this.selectedKey = this.examDetail.checkIds[0];
            } else {
              this.selectedKey = this.examDetail.radioIds[index+1];
            }
          }
        });
      }
      if(currentQuestion.type === '多选题') {
        this.examDetail.checkIds.forEach((item, index) => {
          if(item === currentQuestion.id && index+1 != this.examDetail.checkIds.length) {
              this.selectedKey = this.examDetail.checkIds[index+1];
          }
        });
      }
      
      this.getQuestionDetail(this.selectedKey);
      
      document.getElementById('scroller').scrollBy(0, 43);
    }
  }
}
</script>
