      ...
      <a-layout-sider width="190" :style="{background: '#444',overflow: 'auto', height: '100vh', position: 'fixed', left: 0 }">
        ...
        >
          <a-sub-menu key="question_radio">
            ...
            <a-menu-item v-for="(item, index) in examDetail.radioIds" :key="item" @click="getQuestionDetail(item)" v-show="resultsMap.get(item)==='False'">
          ...
          <a-sub-menu key="question_check">
            ...
            <a-menu-item v-for="(item, index) in examDetail.checkIds" :key="item" @click="getQuestionDetail(item)" v-show="resultsMap.get(item)==='False'">
              <a-icon type="check" v-if="resultsMap.get(item)==='True'"/>
              <a-icon type="close" v-if="resultsMap.get(item)==='False'"/>
              题目{{ index + 1 }}
            </a-menu-item>
